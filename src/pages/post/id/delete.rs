...
pub async fn service(
    ...
    let Ok(current_user) = auth(&state.db_pool, &req).await else {
        let location = format!("/sign-in?back=/post/{post_id}/delete");
    ...
    if !post.is_deletable && query.error.is_none() {
        let location =
            format!("/post/{post_id}/delete?error=You are not allowed to delete this post");
    ...
    lazy_static! {
        static ref TEMPLATE: Template = {
            let template = include_str!("../../../../templates/post/id/delete.html");
